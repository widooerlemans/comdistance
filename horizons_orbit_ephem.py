...
- For each comet:
    * Fetches osculating elements from JPL SBDB via sbdb_elements()
      with a fallback to Horizons elements().
      Then augments them with a, Q, orbital period, mean motion.
    * Builds a 15-day daily ephemeris via JPL Horizons with RA/DEC
      and predicted magnitude (if possible).
...
from horizons_pull import (
    ...
    sbdb_elements,
    horizons_elements,  # ✅ fallback for orbits
    QUANTITIES,
)
...
def sbdb_orbit_extended(label: str) -> Optional[Dict[str, Any]]:
    ...
    2. Fallback to Horizons elements() with numeric record id if possible
    3. Enrich with:
        - a_au, Q_au
...
def build_ephemeris_span(designation: str, observer: str, days: int = DAYS) -> List[Dict[str, Any]]:
    ...
    for row in eph:
        # r_au from Horizons ephemerides (column 'r' in QUANTITIES)
        try:
            r_au = float(row["r"])
        except Exception:
            r_au = None
