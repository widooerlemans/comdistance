...
def sbdb_orbit_extended(label: str) -> Optional[Dict[str, Any]]:
    """
    Wrap your existing sbdb_elements() and enrich it with:
      - a_au, Q_au
      - period_days, period_years
      - n_deg_per_day (mean motion)
    whenever possible.

    We **do not** remove or rename any fields sbdb_elements() already sets;
    we only add extra, derived values.
    """
    ...
    # Bound orbits only for the derived quantities
    ...
    # If SBDB didn't specify a solution/reference, mirror horizons_pull defaults
...
def build_ephemeris_span(designation: str, observer: str, days: int = DAYS) -> List[Dict[str, Any]]:
    ...
    for row in eph:
        # r_au from Horizons ephemerides
        try:
            r_au = float(row["r"])
        except Exception:
            r_au = None
...
def fetch_orbit_and_ephem(comet_id: str, observer: str) -> Dict[str, Any]:
    ...
    For a comet designation:
      - fetch extended orbit from SBDB
      - build 15-day Horizons ephemeris
      - derive v_pred_now from the first ephemeris row
    """
